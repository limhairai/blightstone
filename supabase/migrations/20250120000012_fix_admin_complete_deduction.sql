-- Fix admin complete functionality to actually deduct money from wallet
-- Currently it only releases reserved funds but doesn't deduct from balance
...
'FIXED: Deducts money from wallet balance AND releases reserved funds when admin completes topup request.';

-- Log the fix
DO $$
BEGIN
    RAISE NOTICE 'Fixed complete_topup_transfer: Now deducts from balance_cents AND releases reserved_balance_cents';
    RAISE NOTICE 'When admin marks topup as completed, money will properly leave the client wallet';
END $$; 