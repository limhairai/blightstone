-- Fix ambiguous created_at reference in get_tickets_with_metadata function
...
BEGIN
    ...
    LEFT JOIN (
        SELECT 
            ...
            (SELECT content FROM public.support_ticket_messages WHERE support_ticket_messages.ticket_id = m.ticket_id ORDER BY support_ticket_messages.created_at DESC LIMIT 1) as last_message_content,
            (SELECT sender_id FROM public.support_ticket_messages WHERE support_ticket_messages.ticket_id = m.ticket_id ORDER BY support_ticket_messages.created_at DESC LIMIT 1) as last_message_sender,
