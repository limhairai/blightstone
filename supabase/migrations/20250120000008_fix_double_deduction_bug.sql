-- Fix double deduction bug in complete_topup_transfer function
-- The function was deducting from both balance_cents AND reserved_balance_cents
-- This caused double charging: reserved funds should only be "released", not deducted from balance again
...
BEGIN
    ...
    -- FIXED: Only release reserved funds, don't deduct from balance again
    -- The funds were already "taken" when they were reserved
...
COMMENT ON FUNCTION public.complete_topup_transfer(UUID, INTEGER, UUID) IS 'FIXED: Completes topup by releasing reserved funds only (not double deducting from balance)'; 