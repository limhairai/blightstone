...
interface FormData {
  ...
  cardGroup?: string;
  [key: string]: Account[] | string | number | boolean | undefined;
}

interface MultiAccountFormProps {
  formData: FormData;
...
export function MultiAccountForm({ formData, onChange, isSubmitting, onSubmit }: MultiAccountFormProps) {
  ...
  // Shared field change
  const handleChange = (field: keyof Omit<FormData, 'accounts'>, value: string | number | boolean | undefined) => {
  ...
  const handleAccountChange = (idx: number, field: keyof Omit<Account, 'id'>, value: string) => {
