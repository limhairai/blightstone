...
interface BatchAccountFormProps {
  formData: BatchFormData; // Use specific type
  onChange: (data: BatchFormData) => void; // Use specific type
}

export function BatchAccountForm({ formData, onChange }: BatchAccountFormProps) {
  const [localData, setLocalData] = useState<BatchFormData>({
  ...
  const handleChange = (field: keyof Omit<BatchFormData, 'accounts'>, value: string) => { // More specific field type
    setLocalData((prev) => ({
      ...prev,
      [field]: value,
    }))
  ...
  const handleAccountChange = (id: number, field: keyof Omit<AccountDetail, 'id'>, value: string) => { // More specific field type
    setLocalData((prev) => ({
      ...prev,
      accounts: prev.accounts.map((account: AccountDetail) => (account.id === id ? { ...account, [field]: value } : account)),
    }))
  }

  const addAccount = () => {
    setLocalData((prev) => {
      const newId = prev.accounts.length > 0 ? Math.max(...prev.accounts.map((a: AccountDetail) => a.id)) + 1 : 1;
      const newData = {
        ...prev,
        accounts: [
          ...prev.accounts,
  ...
  const removeAccount = (id: number) => {
    if (localData.accounts.length > 1) {
      setLocalData((prev) => {
        const newData = {
          ...prev,
          accounts: prev.accounts.filter((account: AccountDetail) => account.id !== id),
        }
  ...
  useEffect(() => {
    const accountsChanged =
      ...
      !formData.accounts.every((formAcc, index) => {
        const localAcc = localData.accounts[index];
        return localAcc && formAcc.id === localAcc.id && formAcc.name === localAcc.name && formAcc.landingPageUrl === localAcc.landingPageUrl && formAcc.facebookPageUrl === localAcc.facebookPageUrl;
      });
  ...
  }, [formData]); // localData is intentionally omitted for comparison as per original comment
  ...
  return (
    <div className="space-y-5">
      ...
      {localData.accounts.map((account: AccountDetail, index: number) => (
