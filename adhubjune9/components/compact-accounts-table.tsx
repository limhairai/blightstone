...
import { WithdrawBalanceDialog } from "@/components/withdraw-balance-dialog"
import { AccountTransactionsDialog } from "@/components/account-transactions-dialog"
import { BulkTopUpDialog } from "@/components/bulk-top-up-dialog"
import { MOCK_ACCOUNTS } from "@/data/mock-accounts"
import { MOCK_BUSINESSES } from "@/data/mock-businesses"
import { formatCurrency } from "@/utils/format"
import { MoreHorizontal, Eye, ArrowUpRight, ArrowDownLeft, Wallet, Pause, Play, Copy } from "lucide-react"
...
export function CompactAccountsTable({ initialBusinessFilter = "all" }: CompactAccountsTableProps) {
  const [selectedAccounts, setSelectedAccounts] = useState<string[]>([])
  const [searchQuery, setSearchQuery] = useState("")
  const [statusFilter, setStatusFilter] = useState<string>("all")
  const [businessFilter, setBusinessFilter] = useState<string>(initialBusinessFilter)
  const [viewDetailsAccount, setViewDetailsAccount] = useState<MockAccount | null>(null)
  const [topUpAccount, setTopUpAccount] = useState<MockAccount | null>(null)
  const [withdrawAccount, setWithdrawAccount] = useState<MockAccount | null>(null)
  const [transactionsAccount, setTransactionsAccount] = useState<MockAccount | null>(null)
  ...
  const transformedAccounts = useMemo(() => {
    return MOCK_ACCOUNTS.map((account) => {
      // Find the business to get BM ID
      const business = MOCK_BUSINESSES.find((b) => b.name === account.business)
      ...
      return {
        ...
        currency: "USD",
        bmId: business?.bmId || null,
        quotaUsage: Math.round((account.spent / account.quota) * 100), // Calculate quota usage percentage
      }
    })
  }, [])
  ...
  const handleRowClick = (account: MockAccount, e: React.MouseEvent) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {selectedAccounts.length > 0 && (
        <div className="flex items-center gap-2 p-3 bg-muted rounded-lg border border-border">
          ...
          <Button
            variant="outline"
            size="sm"
            className="ml-auto border-border text-foreground hover:bg-accent"
      ...
      <TopUpDialog
        ...
        open={!!topUpAccount}
        onOpenChange={(open) => !open && setTopUpAccount(null)}
      />
      ...
      <AccountTransactionsDialog
        ...
        open={!!transactionsAccount}
        onOpenChange={(open) => !open && setTransactionsAccount(null)}
      />
      <BulkTopUpDialog
        selectedAccounts={selectedAccounts.map((id) => filteredAccounts.find((acc) => acc.id === id)!).filter(Boolean)}
        open={bulkTopUpOpen}
