...
export function DistributeFunds({ walletBalance, onDistribute }: DistributeFundsProps) {
  ...
  // Use centralized mock accounts data
  ...
  useEffect(() => {
    if (equalDistribution && distributions.length > 0) {
      ...
      const numItems = distributions.length
      const equalAmount = (amountValue / numItems).toFixed(2)
      const equalPercentage =
        amountValue > 0 && numItems > 0 ? ((Number.parseFloat(equalAmount) / amountValue) * 100).toFixed(0) : "0"
